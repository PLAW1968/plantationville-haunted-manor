(()=>{
 ...
 document.querySelectorAll('[data-carousel]').forEach(c=>{const slides=[...c.querySelectorAll('.memory-slide')],dots=c.querySelector('.carousel-dots');if(!slides.length)return;let i=0,t;slides.forEach((_,x)=>{const b=document.createElement('button');b.type='button';b.setAttribute('aria-label','Show item '+(x+1));b.onclick=()=>show(x,true);dots.appendChild(b)});const db=[...dots.children];function show(x,user=false){i=(x+slides.length)%slides.length;slides.forEach((el,j)=>{el.classList.toggle('active',j===i);const v=el.querySelector('video');if(v&&j!==i)v.pause()});db.forEach((el,j)=>el.classList.toggle('active',j===i));if(user)restart()}function restart(){clearInterval(t);t=setInterval(()=>show(i+1),Number(c.dataset.delay)||5000)}c.querySelector('.prev').onclick=()=>show(i-1,true);c.querySelector('.next').onclick=()=>show(i+1,true);let touchX=null,touchY=null;c.addEventListener('touchstart',e=>{if(e.touches.length!==1)return;touchX=e.touches[0].clientX;touchY=e.touches[0].clientY;clearInterval(t)},{passive:true});c.addEventListener('touchend',e=>{if(touchX===null||!e.changedTouches.length){restart();return}const dx=e.changedTouches[0].clientX-touchX,dy=e.changedTouches[0].clientY-touchY;touchX=touchY=null;if(Math.abs(dx)>45&&Math.abs(dx)>Math.abs(dy)*1.25)show(i+(dx<0?1:-1),true);else restart()},{passive:true});show(0);restart();c.addEventListener('mouseenter',()=>clearInterval(t));c.addEventListener('mouseleave',restart)});
 ...
 document.querySelectorAll('[data-story-carousel]').forEach(carousel=>{
   ...
   carousel.addEventListener('touchstart',e=>{if(e.touches.length!==1)return;touchX=e.touches[0].clientX;touchY=e.touches[0].clientY},{passive:true});
   carousel.addEventListener('touchend',e=>{if(touchX===null||!e.changedTouches.length)return;const dx=e.changedTouches[0].clientX-touchX,dy=e.changedTouches[0].clientY-touchY;touchX=touchY=null;if(Math.abs(dx)>45&&Math.abs(dx)>Math.abs(dy)*1.25){index=(index+(dx<0?1:-1)+cards.length)%cards.length;render()}},{passive:true});
   render();
 });
